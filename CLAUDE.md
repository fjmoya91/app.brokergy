...
48.k **La envolvente se empieza ya en la OPORTUNIDAD** (`/envolvente/<id_oportunidad>?origen=op`, botón con el logo CE3X en la fila de iconos de Resultados de la calculadora, solo STAFF y con la oportunidad guardada). Tercer origen de la misma ventana, con el mismo patrón que el CEE directo (regla 48.i): **se ADAPTA la fila** ([logic/oportunidad.js](implementation/frontend/src/features/cee-envolvente/logic/oportunidad.js), `oportunidadComoExpediente`, cargado también por el backend) componiendo la `instalacion` con los MISMOS campos que siembra `expedienteService` al aceptar. **Lo señalado se guarda en `oportunidades.datos_calculo.envolvente_cee`**, con las MISMAS claves que `expedientes.cee` (`envolvente`, `envolvente_fotos`, `envolvente_imagenes`), por la RPC `set_oportunidad_cee_field` (`scripts/cee_envolvente_oportunidad.sql`, ya aplicada) — nunca read-modify-write de `datos_calculo` (regla 19). **Al aceptar, `expedienteService` las vuelca a `cee`** y se sigue donde se dejó; los ficheros de Drive no se mueven porque la carpeta de la oportunidad ES la del expediente. **REGLA — el guardado de la calculadora (`POST /api/oportunidades`) conserva SIEMPRE el `envolvente_cee` de la BD**: la copia del navegador no lo trae al día (la ventana está en otra pestaña) y re-guardar la simulación borraría las ventanas recién puestas. **REGLA — una oportunidad ya aceptada no se escribe**: la ventana salta sola al expediente (`GET /:id/oportunidad` devuelve `expediente`), y cargarla o guardar su trabajo responde 409 — dos envolventes del mismo edificio es lo que no puede pasar. **En la oportunidad NO se genera el `.cex`** (409, y la pestaña lo explica): no hay técnico que lo firme ni número de expediente con el que nombrarlo, y el final se busca por ese nombre. Tampoco se lee la placa (escribe en una `instalacion` que aún no existe) ni se corrige el técnico. `origen=op` es **solo del equipo interno** (`staffSiOportunidad`): el certificador no tiene oportunidades asignadas y la fila lleva el margen. La fila se proyecta por JSON path (`filaOportunidad`), nunca `datos_calculo` entero (regla 22). ⚠️ Lo que se marque en «qué construcciones cuentan» desde la envolvente se escribe en `inputs` de la oportunidad, y si la calculadora está abierta en otra pestaña con su copia vieja, el siguiente «Guardar» de la calculadora lo pisa.
