...
import { at, caja, centro, centroide, cota, CAMARA_ISO, ESCALA_AXO, fmt, largo,
         pegarAPared, proyector, recorrido, reparto, TOPE_ALT }
...
//: el dedo en una tablet. Es más alta que el muro a propósito.
const ASA = 0.6;

//: El tirador de una pared, en metros. Solo sale en la pared SELECCIONADA: si
//: cualquier pared se pudiera arrastrar sin más, mover el plano con el puntero
//: encima de una la movería sin querer — y eso cambia una superficie que va al
//: certificado.
const TIRADOR = 0.30;

//: Hasta dónde tira el imán al soltar un extremo, en metros. Es generoso a
//: propósito —un ancho de puerta— porque lo que se dibuja va DE PARED A PARED:
//: a esa distancia la intención ya es esa, y soltarlo a 20 cm de la pared deja
//: un tabique que no llega a ninguna parte. Y si hay dos cerca gana la más
//: próxima, así que de más no se equivoca: solo alcanza más lejos.
const IMAN = 1.6;
...
export function PlanoPlanta({ planta, plano, capas: capasPedidas, entorno, onEntorno,
    ...
    useEffect(() => {
        ...
        const rueda = (e) => {
            ...
            escalar(e.deltaY < 0 ? 1 / ZOOM.paso : ZOOM.paso, p.x, p.y);
        };
    ...
    const pegar = (x, y, salvo) =>
        pegarAPared(murosDe(planta), x, y, IMAN, salvo) || { x, y, id: null };
    ...
    const onMove = (e) => {
        ...
        if (d.gesto === 'nueva') {
            const p = aDibujo(e.clientX, e.clientY, d.vb);
            setTrazo({ a: d.a, b: pegar(p.x, p.y, null) });
            return;
        }
    ...
    const onUp = (e) => {
        ...
        if (d?.gesto === 'nueva') {
            ...
            // decide `dibujaPared`, que es quien la da de alta.
            if (t) dibujaPared(planta.id, [t.a.x, t.a.y], [t.b.x, t.b.y]);
    ...
    // Un tamaño de letra en METROS, proporcional a lo que se está VIENDO: así
    // se lee igual en una casa de 8 m que en una de 40, y no crece al ampliar.
    ...
    return (
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-3">
            <Controles
                ...
                es3d={es3d}
                dibujando={dibujando}
                onDibujar={d => { setDibujando(d); setTrazo(null); }}
                ...
                <svg ref={svgRef}
                    ...
                    {es3d ? (
                        ...
                        <>
                            ...
                            <Tiradores muro={capa2d.find(m => m.id === sel)}
            ...
            <Leyenda es3d={es3d} dibujando={dibujando} />
        </div>
    );
}

/** La cabecera del plano: qué se está viendo, y los mandos para verlo. */
function Controles({ titulo, subtitulo, es3d, onGirar, dibujando, onDibujar,
            ...
            <div className="ml-auto flex items-center gap-1.5">
                ...
                {!es3d && onDibujar && (
                    <Boton onClick={() => onDibujar(!dibujando)} activo={dibujando}
                        ...
                        {dibujando ? '✎ Suelta de pared a pared' : '✎ Pared nueva'}
                    </Boton>
                )}
                {es3d && onGirar && (
                    <>
 ...
 */
function Tiradores({ muro, onCoger }) {
    ...
    return (
        <g>
            {[0, pts.length - 1].map(i => (
                <circle key={i} cx={pts[i][0]} cy={pts[i][1]} r={TIRADOR}
                        fill={PAPEL} stroke="var(--brand-primary)" strokeWidth={0.09}
                        className="cursor-move" onPointerDown={agarre(i)}>
                    <title>Arrastra este extremo</title>
                </circle>
            ))}
            <circle cx={medio[0]} cy={medio[1]} r={TIRADOR * 0.8}
    ...
    return (
        <g style={{ pointerEvents: 'none' }}>
            <line x1={a.x} y1={a.y} x2={b.x} y2={b.y}
                  stroke={COLOR_TIPO.PARTICION_VERTICAL} strokeWidth={0.22}
                  strokeDasharray="0.5 0.3" strokeLinecap="round" />
            {[a, b].map((q, i) => (
                <circle key={i} cx={q.x} cy={q.y} r={0.22}
                        fill={q.id ? COLOR_TIPO.PARTICION_VERTICAL : PAPEL}
                        stroke={COLOR_TIPO.PARTICION_VERTICAL} strokeWidth={0.08} />
            ))}
            <rect x={(a.x + b.x) / 2 - tam * 1.5} y={(a.y + b.y) / 2 - tam * 1.5}
                  width={tam * 3} height={tam * 1.25} rx={tam * 0.18}
                  fill={PAPEL} opacity={0.85} />
            <text x={(a.x + b.x) / 2} y={(a.y + b.y) / 2 - tam * 0.6}
                  fontSize={tam} fontWeight={800} textAnchor="middle"
                  fill="var(--text-primary)">{fmt(L)} m</text>
        </g>
    );
}
...
function GloboHueco({ h, pared, tip }) {
    ...
    return (
        <div className="pointer-events-none absolute z-10 min-w-[152px] rounded-xl border
             ...
             style={{ left, top }}>
            <div className="flex items-baseline gap-2">
                <b className="text-[12px] font-black tracking-wide">{h.nombre}</b>
...
function Leyenda({ es3d, dibujando }) {
    return (
        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1.5 rounded-xl border
            ...
            <span className="ml-auto text-[11px] text-white/30">
                {dibujando
                    ? 'arrastra de una pared a otra · se mide sola'
                    : es3d
