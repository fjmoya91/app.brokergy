...
export function FichaRes080Modal({ isOpen, onClose, expediente, results, onSaveDrive }) {
    const { user } = useAuth();
    ...
    const beneficioStr = Math.round((results?.ahorroEnergiaFinalTotal || results?.savingsKwh || 0) * (results?.price_kwh || 0.10)).toLocaleString('es-ES');
    ...
    // BUILD HTML PARA PDF
    // ═══════════════════════════════════════════════════════════════════════════
    const buildStaticHtml = () => `<!DOCTYPE html><html><head><meta charset="UTF-8">
<style>${PDF_CSS}</style>
</head><body>
...
<div class="fn"><sup>5</sup> Para la elaboración del certificado se debe emplear una herramienta informática de las registradas como documentos reconocidos para la certificación de la eficiencia energética de los edificios.</div>
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            ...
            a.download = `${numexpte || 'DRAFT'} - Ficha RES080.pdf`;
            a.click();
    ...
    const handleSendByEmail = async () => {
        ...
        try {
            ...
            if (response.data.success) alert(`✅ Ficha RES080 enviada correctamente a ${toEmail}`);
        } catch (error) {
    ...
    const handleSendByWhatsapp = async () => {
        ...
        try {
            ...
            await axios.post('/api/whatsapp/send-media', {
                ...
                media: { base64: pdfResp.data?.pdf, filename: `${numexpte}_Ficha_RES080.pdf`, mimetype: 'application/pdf' },
                asDocument: true,
            });
            alert(`✅ Ficha RES080 enviada por WhatsApp correctamente.`);
        } catch (error) {
                ...
                <div className="flex-shrink-0 flex items-center justify-between px-5 py-3 border-b border-white/[0.07]">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="border-l border-white/10 pl-3">
                            <h2 className="text-sm font-black text-white tracking-wider uppercase">Ficha RES080</h2>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="hidden sm:flex items-center gap-4 mr-3 pr-3 border-r border-white/10">
                            <div className="text-center">
                                <div className="text-white/60 font-black text-sm">{eFiStr} kWh</div>
                                <div className="text-white/25 text-[10px] uppercase tracking-wider">EF inicial</div>
                            </div>
                            <div className="text-center">
                                <div className="text-brand font-black text-sm">{aeTotalStr} kWh</div>
                        ...
                        <button onClick={handleSendByEmail} disabled={sendingEmail || generating || savingDrive || sendingWhatsapp}
                        ...
                        <button onClick={handleSendByWhatsapp} disabled={sendingWhatsapp || generating || savingDrive || sendingEmail}
                        ...
                        <div className="doc-wrap">
                            ...
                            <div className="doc-page">
                                <table className="doc-table" style={{ marginBottom: 16 }}>
                                    <tbody>
                                        <tr style={{ backgroundColor: '#f2f2f2' }}><td style={{ width: '15%' }}>Ficha</td><td><strong>RES080: Rehabilitación profunda de edificios de viviendas</strong></td></tr>
                                        <tr><td>Código</td><td>RES080</td></tr>
                                        <tr><td>Versión</td><td>V1.1</td></tr>
                                        <tr><td>Sector</td><td>Residencial</td></tr>
                                    </tbody>
                                ...
                                <div className="doc-p">Rehabilitación profunda de edificios existentes de uso residencial privado<sup>1</sup>.</div>

                                <div className="doc-section-num">2. REQUISITOS</div>
                                <div className="doc-p">La rehabilitación debe afectar simultáneamente a la envolvente y al menos a una de las instalaciones térmicas: calefacción, agua caliente sanitaria (ACS), refrigeración, climatización y/o iluminación.</div>
                                ...
                                <div className="doc-formula" style={{ textAlign: 'center', marginLeft: 0 }}>
                                    AE<sub>TOTAL</sub> = F<sub>P</sub> · (EF<sub>i</sub> – EF<sub>f</sub>)
                                </div>

                                <div className="doc-p">Donde:</div>
                                <table className="doc-table table-no-border">
                                    <tbody>
                                        <tr><td style={{ width: '13%', textAlign: 'left' }}>F<sub>P</sub></td><td style={{ textAlign: 'left' }}>Factor de ponderación<sup>2</sup></td><td style={{ width: '16%', textAlign: 'left' }}>1</td></tr>
                                        <tr><td style={{ textAlign: 'left' }}>EF<sub>i</sub></td><td style={{ textAlign: 'left' }}>Consumo de energía final anual del edificio antes de la actuación</td><td style={{ textAlign: 'left' }}>kWh/año</td></tr>
                                        <tr><td style={{ textAlign: 'left' }}>EF<sub>f</sub></td><td style={{ textAlign: 'left' }}>Consumo de energía final anual del edificio después de la actuación</td><td style={{ textAlign: 'left' }}>kWh/año</td></tr>
                                ...
                                <div className="doc-footnote"><sup>2</sup> Factor de ponderación para ajustar el valor de la demanda de energía estimado por métodos reconocidos al valor del consumo real de energía final.</div>
                            </div>
                            ...
                            <div className="doc-page">
                                ...
                                <div className="calc-table-group" style={{ width: '100%', display: 'flex', justifyContent: 'space-between' }}>
                                    <table className="calc-table" style={{ width: '20%' }}>
                                        <thead><tr><th>F<sub>P</sub></th></tr></thead>
                                        <tbody><tr><td>1</td></tr></tbody>
                                    </table>
                                    <table className="calc-table" style={{ width: '26%' }}>
                                        <thead><tr><th>EF<sub>i</sub></th></tr></thead>
                                        <tbody><tr><td>{eFiStr}</td></tr></tbody>
                                    </table>
                                    <table className="calc-table" style={{ width: '26%' }}>
                                    ...
                                    <table className="calc-table" style={{ width: '18%' }}>
                                        <thead><tr><th>AE<sub>TOTAL</sub></th></tr></thead>
                                        <tbody><tr><td>{aeTotalStr}</td></tr></tbody>
                                    </table>
                                ...
                                <table className="doc-table table-no-border" style={{ marginBottom: 12 }}>
                                    <tbody>
                                        <tr className="grey-row">
                                            ...
                                            <td><em>Duración indicativa de la actuación<sup>3</sup></em></td>
                                ...
                                <div className="doc-p" style={{ textIndent: 28 }}>3. Facturas justificativas de la inversión realizada<sup>4</sup> que incluyan una descripción detallada de los elementos principales (por ejemplo, aquellos de cuya ficha técnica se toman datos para calcular el ahorro).</div>
                                <div className="doc-p" style={{ textIndent: 28 }}>4. Informe fotográfico del inmueble antes y después de la actuación con identificación de la superficie e instalaciones afectadas por la actuación.</div>
                                <div className="doc-p" style={{ textIndent: 28 }}>5. Certificado suscrito por el director o responsable de la obra, incluyendo:</div>
                                <div className="doc-p" style={{ textIndent: 56 }}>a) Enumeración y descripción de los elementos o equipos afectados.</div>

                                <div className="doc-spacer" />
                                <hr className="doc-hr"/>
                                <div className="doc-footnote"><sup>3</sup> Según Recomendación (UE) 2019/1658, de la Comisión, de 25 de septiembre, relativa a la transposición de la obligación de ahorro de energía en virtud de la Directiva de eficiencia energética, o en su defecto, a criterio de la persona técnica responsable. Valor requerido para fines administrativos (no utilizado en el cálculo del ahorro de energía).</div>
                                <div className="doc-footnote"><sup>4</sup> Todas las facturas deben contener, como mínimo, los datos y requisitos exigidos por la Agencia Tributaria.</div>
                            </div>

                            {/* ══════════ PÁGINA 3 ══════════ */}
                            <div className="doc-page">
                                <div className="doc-p" style={{ textIndent: 56 }}>b) Certificado/s de fin de obra o de puesta en funcionamiento de los elementos y/o las instalaciones térmicas afectadas.</div>
                                <div className="doc-p" style={{ textIndent: 56 }}>c) Los valores de las variables de la fórmula del cálculo de ahorro energético del apartado 3.</div>
                                <div className="doc-p" style={{ textIndent: 28 }}>6. Copia de la comunicación de puesta en funcionamiento de la instalación térmica sustituida ante el registro habilitado por el órgano competente de la comunidad autónoma.</div>
                                <div className="doc-p" style={{ textIndent: 28 }}>7. Certificado de eficiencia energética del edificio<sup>5</sup>, correspondiente al estado previo al inicio de la rehabilitación, con el justificante de registro.</div>
                                <div className="doc-p" style={{ textIndent: 28 }}>8. Certificado de eficiencia energética del edificio, emitido tras la actuación ejecutada, con justificante de registro, realizada con la misma herramienta informática que la utilizada para el certificado de eficiencia energética previo.</div>

                                <div className="doc-spacer" />
                                <hr className="doc-hr"/>
                                <div className="doc-footnote"><sup>5</sup> Para la elaboración del certificado se debe emplear una herramienta informática de las registradas como documentos reconocidos para la certificación de la eficiencia energética de los edificios.</div>
                            </div>
