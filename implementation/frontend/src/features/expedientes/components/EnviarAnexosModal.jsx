        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-[#0F1013] border border-white/[0.07] rounded-2xl shadow-2xl w-full max-w-lg md:max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                {/* Footer — los canales viven AQUÍ, pegados a ENVIAR: es la última
                    ...
                    cuerpo largo (destinatarios + incidencias + mensaje) no se veían. */}
                <div className="px-5 py-3.5 bg-white/[0.02] border-t border-white/[0.07] flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex items-center gap-2.5 shrink-0">
                        ...
                        <span className={`text-[9px] font-bold uppercase tracking-widest whitespace-nowrap ${(avisoCanal || !sendDocs.length) ? 'text-amber-400/80' : 'text-white/25'}`}>
                            {!sendDocs.length
                                ? (blockedSelected.length ? blockers[blockedSelected[0]] : 'Elige documento')
                                : (avisoCanal || `${sendDocs.length} doc${sendDocs.length === 1 ? '' : 's'}`)}
                        </span>
                        ...
                        <button onClick={handleSend} disabled={busy || !sendDocs.length || (!willEmail && !willWhatsapp)}
                            title={!sendDocs.length ? (blockedSelected.length ? blockers[blockedSelected[0]] : 'Selecciona al menos un documento') : (avisoCanal || 'Enviar')}
