...
// Fuente del diseño (Instrument Sans) auto-alojada en /public/fonts. El documento
// va entero en la MISMA familia, igual que el CIFO: la jerarquía la marcan el peso,
// el tamaño y el color. Ojo: este bloque está DUPLICADO a propósito en res080Doc.js
// (el PDF) — tocar uno obliga a tocar el otro.
...
export function CertificadoRes080Modal({ isOpen, onClose, expediente, results, rechazo = null, attachments: externalAttachments, onAttachmentsChange, onSaveDrive, onSaveFichaLink, onSaveExtraAnnexes, onSaveAnnexPrefs, onMarkSent, onSaveSignedLink, onSaveCe3x }) {
    ...
    const handleManualExtraUploads = async (fileList, labelOverride) => {
        const files = Array.from(fileList || []);
        if (files.length === 0) return;
        for (let i = 0; i < files.length; i++) {
            setExtraProgress({ done: i, total: files.length });
            // El label fijo (EPREL/KEYMARK) solo se aplica al primero: es un hueco único.
            await handleManualExtraUpload(files[i], i === 0 ? labelOverride : undefined);
        }
                ...
                <div className="p-8 grid gap-4 max-h-[65vh] overflow-y-auto custom-scrollbar bg-gradient-to-b from-transparent to-black/20">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
                        ...
                        {eficienciaDocs.map(row => {
                            ...
                            return (
                                <div key={row.kind}
                                     // Soltar encima de este hueco lo sube con SU etiqueta
                                     // (EPREL / KEYMARK) en vez de como anexo suelto.
                                     onDragOver={annex ? undefined : (e) => {
                                         if (!dragHasFiles(e)) return;
                                         e.preventDefault();
                                         e.stopPropagation();
                                         setIsGlobalDragging(false);
                                     ...
                                     onDragLeave={(e) => { e.currentTarget.style.backgroundColor = ''; }}
                                     onDrop={annex ? undefined : (e) => {
                                         if (!dragHasFiles(e)) return;
                                         e.preventDefault();
                                         e.stopPropagation();
                                         e.currentTarget.style.backgroundColor = '';
                                         handleManualExtraUploads(e.dataTransfer.files, row.label);
                                     }}
                                     className="flex items-center justify-between gap-3 py-2.5 border-t border-white/5 first:border-t-0 rounded-lg transition-colors">
                                    ...
                                    <div className="flex gap-2 shrink-0">
                                        ...
                                        {annex ? (
                                            ...
                                            <label className={`p-2.5 bg-white/5 text-white/40 border border-white/10 rounded-xl cursor-pointer hover:bg-brand hover:text-black hover:border-brand transition-all shadow-xl ${bulkUploading ? 'opacity-40 pointer-events-none' : ''}`}>
                                                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 4v16m8-8H4"/></svg>
                        ...
                        >
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest text-white/20">
                                {bulkUploading
                                    ...
                                    : 'Suelta aquí uno o varios PDFs o imágenes para anexarlos'}
                            </p>
                        </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-[#0F1013] border border-white/[0.07] rounded-2xl shadow-2xl flex flex-col overflow-hidden" style={{ width: '98vw', maxWidth: 1020, height: '96vh' }} onClick={e => e.stopPropagation()}>
                ...
                {signOpen && signPdfB64 && (
                    <FirmarConCertificadoModal
                        ...
                        initialPage={2}
                        // Ancla a la línea del NOMBRE, no a "En Tomelloso, a fecha de firma
                        // electrónica": esa es la penúltima línea del bloque, así que el
                        // recuadro caía justo encima del "Fdo.: <nombre>" y lo tapaba.
                        // Colgando de "Fdo." queda [41, 238, 271, 290] pt en la página 2:
                        // 5,4 pt por debajo del nombre y 6,4 pt por encima del bloque
                        // "CÁLCULO DEL AHORRO", que es todo el hueco libre que hay.
                        // Va acotado a la página 2 a propósito: el certificado lleva los
                        // anexos detrás (23 páginas en 26RES080_60) y un "Fdo." suelto en
                        // uno de ellos se llevaría el recuadro a otra página.
                        signatureAnchor={['fdo.@2']}
            ...
            {consolidarAbierto && (
                <ConsolidarFichaModal
                    ...
                    slots={resolveAllFichaSlots(expediente)}
