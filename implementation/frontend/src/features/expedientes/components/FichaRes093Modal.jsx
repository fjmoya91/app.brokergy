...
export function FichaRes093Modal({ isOpen, onClose, expediente, results, onSaveDrive }) {
    const { user } = useAuth();
    ...
    const cifoDates093Modal = calcCifo(doc);
    // Mismos fallbacks que el CIFO (CertificadoCifoModal.jsx) para que la fecha coincida.
    const fechaInicio = formatFecha(cifoDates093Modal.inicio || doc.fecha_inicio_cifo || doc.fecha_visita_cee_inicial);
    const fechaFin    = formatFecha(cifoDates093Modal.fin    || doc.fecha_fin_cifo    || doc.fecha_firma_cee_final);
...
<div class="fn"><sup>11</sup> Todas las facturas deben contener, como mínimo, los datos y requisitos exigidos por la Agencia Tributaria.</div>
    ...
    const formularioOficial = () => fichaFormulario('RES093', expediente, { results });
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            ...
            a.download = `${numexpte || 'DRAFT'} - Ficha RES093.pdf`;
            a.click();
    ...
    const handleSaveToDrive = async () => {
        const fId = op.drive_folder_id || op.datos_calculo?.drive_folder_id || op.datos_calculo?.inputs?.drive_folder_id;
        if (!fId) { alert('No se encontró el identificador de la carpeta de Drive en la oportunidad.'); return; }
        setSavingDrive(true);
        try {
            const { data } = await axios.post('/api/pdf/save-to-drive', {
                ...docPayload(),
                folderId: fId,
                fileName: `${numexpte || 'DRAFT'} - Ficha RES093`,
    ...
    const handleSendByEmail = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                alert(`✅ Ficha RES093 enviada correctamente a ${toEmail}`);
            }
        } catch (error) {
    ...
    const handleSendByWhatsapp = async () => {
        ...
        try {
            ...
            await axios.post('/api/whatsapp/send-media', {
                phone: toPhone,
                caption,
                media: { base64: pdfBase64, filename: `${numexpte}_Ficha_RES093.pdf`, mimetype: 'application/pdf' },
                asDocument: true,
            });

            alert(`✅ Ficha RES093 enviada por WhatsApp correctamente.`);
        } catch (error) {
            ...
            <div className="bg-[#0F1013] border border-white/[0.07] rounded-2xl shadow-2xl flex flex-col overflow-hidden"
                ...
                <div className="flex-shrink-0 flex items-center justify-between px-5 py-3 border-b border-white/[0.07]">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="border-l border-white/10 pl-3">
                            <h2 className="text-sm font-black text-white tracking-wider uppercase">Ficha RES093</h2>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="hidden sm:flex items-center gap-4 mr-3 pr-3 border-r border-white/10">
                            ...
                            <div className="text-center">
                                <div className="text-indigo-400 font-black text-sm">Cb = {cbStr}</div>
                                <div className="text-white/25 text-[10px] uppercase tracking-wider">Bivalencia</div>
                            </div>
                        </div>
                ...
                {formato === 'oficial' && (
                    <div className="flex-1 min-h-0">
                        <DocumentoOficialPreview formulario={formularioOficial()} titulo="Ficha RES093"
                        ...
                        <div className="doc-wrap">
                            ...
                            <div className="doc-page">
                                <table className="doc-table" style={{ marginBottom: 16 }}>
                                    <tbody>
                                        <tr style={{ backgroundColor: '#f2f2f2' }}><td style={{ width: '15%' }}>Ficha</td><td><strong>RES093: Hibridación en modo paralelo de caldera/s de combustión con bomba de calor de accionamiento eléctrico en edificios residenciales ubicados en la zona climática D1, D2 o D3</strong></td></tr>
                                        <tr><td>Código</td><td>RES093</td></tr>
                                        <tr><td>Versión</td><td>V1.0</td></tr>
                                        <tr><td>Sector</td><td>Residencial</td></tr>
                                    </tbody>
                                ...
                                <div className="doc-p">Hibridación en modo paralelo de caldera/s de combustión existente/s en un edificio de uso residencial privado<sup>1</sup>, ubicado en zona climática D1, D2 o D3, con bomba de calor de accionamiento eléctrico tipo aire-aire, aire-agua o agua-agua o combinadas, para la calefacción y/o, agua caliente sanitaria (ACS).</div>
                                <div className="doc-p">En esta ficha no es aplicable las bombas de calor cuyo compresor esté accionado térmicamente.</div>

                                <div className="doc-section-num">2. REQUISITOS</div>
                                <div className="doc-p">La instalación térmica debe disponer de depósito de inercia o acumulador para el suministro de ACS y/o calefacción.</div>
                                <div className="doc-p">Para poder asignar ahorros a cualquiera de los dos servicios previstos en la fórmula del apartado 3, éste debe operar en funcionamiento bivalente paralelo<sup>2</sup>.</div>
                                ...
                                <div className="doc-formula">
                                    &nbsp;&nbsp;&nbsp;AE<sub>TOTAL</sub> = F<sub>P</sub> · [ (D<sub>CAL</sub> · S ) · (<sup>1</sup>/<sub>η<sub>i</sub></sub> - <sup>1</sup>/<sub>SCOP</sub>) + D<sub>ACS</sub> · (<sup>1</sup>/<sub>η<sub>i</sub></sub> - <sup>1</sup>/<sub>SCOP<sub>dhw</sub></sub>)] · C<sub>b</sub>
                                </div>

                                <div className="doc-spacer" />
                                <hr className="doc-hr"/>
                                <div className="doc-footnote"><sup>1</sup> "Uso residencial privado" según el Anejo A "Terminología" del CTE DB HE (Documento Básico de Ahorro de Energía").</div>
                                <div className="doc-footnote"><sup>2</sup> Es decir, la instalación hidráulica y el sistema de control deben haberse ejecutado especialmente para cada uno de los servicios para los que se consignen ahorros, buscando el aprovechamiento de los generadores con la máxima eficiencia para la/s bomba/s de calor, de tal modo que ésta/s trabaje/n de manera constante contra el punto más frío de la instalación y aportando la/s caldera/s sólo la energía necesaria para alcanzar la temperatura de consigna de impulsión, cuando sea requerida.</div>
                            </div>
                            ...
                            <div className="doc-page">
                                ...
                                <table className="doc-table table-no-border">
                                    <tbody>
                                        <tr><td style={{ width: '13%', textAlign: 'left' }}>F<sub>P</sub></td><td style={{ textAlign: 'left' }}>Factor de ponderación<sup>3</sup></td><td style={{ width: '16%', textAlign: 'left' }}>1</td></tr>
                                        <tr><td style={{ textAlign: 'left' }}>D<sub>cal</sub></td><td style={{ textAlign: 'left' }}>Demanda de energía en calefacción del edificio según certificado de eficiencia energética antes de la actuación</td><td style={{ textAlign: 'left' }}>kWh/m<sup>2</sup> · año</td></tr>
                                        <tr><td style={{ textAlign: 'left' }}>S</td><td style={{ textAlign: 'left' }}>Superficie útil habitable del edificio<sup>1</sup></td><td style={{ textAlign: 'left' }}>m<sup>2</sup></td></tr>
                                        <tr><td style={{ textAlign: 'left' }}>D<sub>ACS</sub></td><td style={{ textAlign: 'left' }}>Demanda de energía<sup>4</sup> térmica en agua caliente sanitaria del edificio según certificado de eficiencia energética antes de la actuación o alternativamente conforme al anexo F del DB HE1 del CTE</td><td style={{ textAlign: 'left' }}>kWh/año</td></tr>
                                        <tr><td style={{ textAlign: 'left' }}>η<sub>i</sub></td><td style={{ textAlign: 'left' }}>Rendimiento de caldera sobre energía referido<sup>5</sup> al PCS<sup>6</sup></td><td style={{ textAlign: 'left' }}>(en tanto por uno)</td></tr>
                                        <tr><td style={{ textAlign: 'left' }}>SCOP</td><td style={{ textAlign: 'left' }}>Coeficiente de rendimiento estacional de la bomba de calor, en calefacción<sup>7</sup></td><td style={{ textAlign: 'left' }}></td></tr>
                                        <tr><td style={{ textAlign: 'left' }}>SCOP<sub>dhw</sub></td><td style={{ textAlign: 'left' }}>Coeficiente de rendimiento estacional de la bomba de la bomba de calor en ACS<sup>8</sup></td><td style={{ textAlign: 'left' }}></td></tr>
                                        <tr><td style={{ textAlign: 'left' }}>C<sub>b</sub></td><td style={{ textAlign: 'left' }}>Coeficiente de cobertura por bivalencia<sup>9</sup> en paralelo</td><td style={{ textAlign: 'left' }}>(en tanto por uno)</td></tr>
                                ...
                                <div className="calc-table-group" style={{ width: '100%', display: 'flex', justifyContent: 'space-between' }}>
                                    <table className="calc-table" style={{ width: '67%' }}>
                                        <thead>
                                            <tr><th>F<sub>p</sub></th><th>D<sub>CAL</sub></th><th>S</th><th>D<sub>ACS</sub></th><th>η<sub>i</sub></th><th>SCOP</th><th>SCOP<sub>dhw</sub></th><th>C<sub>b</sub></th></tr>
                                        </thead>
                                        <tbody>
                                    ...
                                    <table className="calc-table" style={{ width: '14%' }}>
                                        <thead>
                                            <tr><th>AE<sub>TOTAL</sub></th></tr>
                                        </thead>
                                        <tbody>
                                    ...
                                    <table className="calc-table" style={{ width: '7%' }}>
                                        <thead>
                                            <tr><th><em>D<sub>i</sub></em></th></tr>
                                        </thead>
                                        <tbody>
                                ...
                                <div className="doc-spacer" />
                                <hr className="doc-hr"/>
                                <div className="doc-footnote"><sup>3</sup> Factor de ponderación para ajustar el valor de la demanda de energía estimado por métodos reconocidos al valor del consumo real de energía final.</div>
                                <div className="doc-footnote"><sup>4</sup> Alternativamente, en caso de sustitución parcial, por ejemplo, cuando se realiza un precalentamiento de un depósito previo al depósito de consumo, se considerará sólo la demanda de energía térmica necesaria para el precalentamiento. Esto es, el calentamiento desde la temperatura de red (DB HE4 del CTE) a la temperatura de consigna deseada en ese depósito.</div>
                                <div className="doc-footnote"><sup>5</sup> Para la conversión de PCI a PCS se usará la formula (PCS = PCI x F<sub>conv</sub>). Para gas natural se utilizará el factor de conversión de F<sub>conv</sub> = 1,106, para gasóleo F<sub>conv</sub> = 1,059, para propano F<sub>conv</sub>= 1,087 y para butano F<sub>conv</sub>= 1,083, según Tabla CB-01 Poderes caloríficos de los combustibles del documento "Diseño de centrales de calor eficientes". https://www.idae.es/uploads/documentos/documentos_11_Guia_tecnica_de_diseno_de_centrales_de_calor_eficientes_e53f312e.pdf</div>
                                <div className="doc-footnote"><sup>6</sup> O alternativamente el valor de la última inspección.</div>
                                <div className="doc-footnote"><sup>7</sup> Ver Anexo II. En caso de secuencia de varias bombas de calor, el SCOP utilizado en esta expresión será el ponderado, en el caso de ser de diferentes características.</div>
                                <div className="doc-footnote"><sup>8</sup> Ver Anexo II de condiciones generales para cálculo de la eficiencia estacional anual en lo relativo al calentamiento de ACS.</div>
                                <div className="doc-footnote"><sup>9</sup> El coeficiente de cobertura por bivalencia es el porcentaje de la demanda de energía térmica anual cubierta por bombas de calor cuando está combinada con generadores auxiliares (calderas) formando un sistema híbrido. Ver Anexo III. En caso de sustitución total C<sub>b</sub> = 1. El valor se expresará en tanto por uno con tres decimales.</div>
                            </div>
                            ...
                            <div className="doc-page">
                                ...
                                <div className="doc-section-num">5. DOCUMENTOS PARA LA JUSTIFICACIÓN DE LOS AHORROS DE LA ACTUACIÓN Y DE SU REALIZACIÓN</div>
                                ...
                                <div className="doc-p" style={{ textIndent: 28 }}>4. Informe fotográfico del conjunto caldera/s y la/s bomba/s de calor antes y después de la actuación con identificación de los equipos afectados.</div>
                                <div className="doc-p" style={{ textIndent: 28 }}>5. Copia de la comunicación de la puesta en servicio presentada en el registro habilitado por el órgano competente de la comunidad autónoma.</div>
