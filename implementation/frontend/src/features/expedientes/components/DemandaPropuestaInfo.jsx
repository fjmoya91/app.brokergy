...
import { compararDemanda, fmtKwh, fmtDec, fmtPct } from '../logic/demandaPropuesta';
...
function Fila({ label, unidad, prop, cee, deltaPct, baja }) {
    return (
        <div className={`${COLS} py-2 border-b border-white/[0.06] last:border-0`}>
            ...
            <span className="text-right whitespace-nowrap">
                <span className={`font-mono text-[13px] font-bold ${baja ? 'text-red-400' : 'text-white'}`}>
                    {cee > 0 ? fmtDec(cee) : '—'}
                </span>
                {deltaPct != null && (
                    ...
                    Math.abs(deltaPct) < 0.05 ? (
                        ...
                        <span className={`ml-1 text-[10px] font-black ${baja ? 'text-red-400' : 'text-emerald-400'}`}>
                            {fmtPct(deltaPct)}
                        </span>
                    )
                )}
            ...
            {cmp ? (
                <>
                    ...
                    <Fila label="Demanda" unidad="kWh/m²·año"
                          ...
                          deltaPct={cmp.demanda.deltaPct} baja={cmp.demanda.baja} />
                    <Fila label="Superficie" unidad="m²"
                          prop={cmp.superficie.prop} cee={cmp.superficie.cee}
                          deltaPct={cmp.superficie.deltaPct} baja={cmp.superficie.baja} />
                        ...
                        <div className="mt-2.5 px-2.5 py-2 rounded-lg bg-red-500/10 border border-red-500/25">
                            <p className="text-[10px] text-red-300/90 leading-snug">
                                <b className="text-red-400">
                                    {cmp.demanda.baja && cmp.superficie.baja ? 'La demanda y la superficie certificadas son menores'
                                        : cmp.demanda.baja ? 'La demanda certificada es menor'
...
export function DemandaPropuestaInfo({ sectionDemand, prop, section, esReforma }) {
    ...
    const cmp = compararDemanda(sectionDemand, prop);
