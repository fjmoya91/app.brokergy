 ...
 * @returns {string}
 */
export function buildCertMessage({ espera, tono, fase, certName, clienteNombre, numExp, expedienteId, dias = null }) {
...
export function buildCertApproveMessage(section, certName, clienteNombre, numExp, ceeFolderLink, expedienteId, priority = 'normal') {
    const tecnico = firstNameProper(certName) || 'técnico';
    const cli = clienteNombre ? ` (${toTitleCase(clienteNombre)})` : '';
