...
def test_la_altura_por_defecto_va_marcada_como_no_medida(corrida):
    ...
    sur = _por_id(res)["FBS1"]
    assert sur.alto.evidence_type.value == "MANUAL"
    assert sur.alto.source.value == "DEFAULT"
    assert sur.requiere_revision is True
    assert "ALTO NO MEDIDO" in sur.nota
    assert sur.largo.evidence_type.value == "MEASURED"
...
def test_el_mapa_y_el_plano_rotulan_con_el_id_del_csv(corrida):
    """Si el mapa dice PB-S003_1 y el CSV dice MBN1, no se pueden cruzar."""
    res, ficheros, _ = corrida
    html = Path(ficheros["debug_map.html"]).read_text(encoding="utf-8")
