...
_PREFIJO = {TIPO_MURO_FACHADA: "F", TIPO_MEDIANERA: "M",
            TIPO_PARTICION_VERTICAL: "PV", TIPO_SUELO: "SU",
...
def _letra_planta(nivel: int | None) -> str:
    """La planta dentro del ID: PB -> B, P1 -> 1, S1 -> S1."""
    if nivel is None:
        return "?"
    if nivel == 0:
        return "B"
    return f"S{abs(nivel)}" if nivel < 0 else str(nivel)


def identificador(tipo: str, nivel: int | None, orientacion: str | None,
                  contador: dict[str, int]) -> str:
    """El ID que se ve en el plano, en el CSV y en CE3X.

    El criterio es el del certificador: **FBSO1 = Fachada, Baja, SurOeste, la
    primera**. Si hubiera otra fachada baja al suroeste seria FBSO2.

        FBSO1   fachada de planta baja al suroeste
        F1NO2   fachada de planta primera al noroeste, la segunda
        MBN1    medianera de planta baja al norte
        CUB1    cubierta de planta baja

    Se numera SIEMPRE, tambien cuando solo hay una: asi el id no cambia de
    forma al aparecer una segunda, que es cuando se rompen las referencias
    cruzadas entre el plano, la tabla y el .cex.
    """
    pref = _PREFIJO[tipo]
    trozo = f"{pref}{_letra_planta(nivel)}{orientacion or ''}"
    contador[trozo] = contador.get(trozo, 0) + 1
    return f"{trozo}{contador[trozo]}"
...
def verticales(tramos: list[Tramo], alto: Traced, planta: Planta | None,
    ...
    for t in tramos:
        ...
        nivel = planta.nivel if planta else None
        eid = identificador(tipo, nivel, t.segment.orientation, contador)
...
def horizontales(elems: list[ElementoHorizontal],
    ...
    for e in elems:
        ...
        salida.append(ElementoCE3X(
            id=identificador(tipo, e.nivel, None, contador),
            planta=e.planta, nivel=e.nivel,
