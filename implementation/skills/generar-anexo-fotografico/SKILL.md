...
> **Versión:** 2026-07-22 · v3 — numeración de ficheros desde Drive (evita duplicados), alcance
> auto-derivado de la Envolvente en RES080, y respeto a los ajustes manuales del anexo.
...
```sql
select e.id exp_id, e.numero_expediente, o.ficha,
       e.documentacion->'facturas'           as facturas,
       e.documentacion->'envolvente'         as envolvente,          -- alcance real del RES080
       e.documentacion->'anexo_excluidas'    as anexo_excluidas,     -- ajustes manuales: respétalos
       e.documentacion->'anexo_orden'        as anexo_orden,
       e.documentacion->'anexo_comentarios'  as anexo_comentarios,
       o.datos_calculo as op_datos_calculo
from expedientes e join oportunidades o on o.id = e.oportunidad_id
where e.numero_expediente = :num;
```
No pidas `documentacion` ni `datos_calculo` enteros: son JSONB grandes y Postgres los descomprime
completos aunque solo mires un subcampo (regla 22 del proyecto).
...
- `presentes[]` — slots que YA tienen foto en `12. DOCUMENTOS PARA CEE`, cada uno con `count`,
  **`nombres[]`** (los nombres de fichero reales que hay en Drive) y **`siguiente_indice`** (el número
  por el que debes continuar al copiar más fotos a ese slot). Úsalos en el paso 3.
...
**Si una actuación justificada NO tiene slot en `slots[]`** (p.ej. la factura incluye ventanas pero no
aparece `FOTO_VENTANAS_ANTES/DESPUES`): **no inventes el nombre del slot ni copies nada**, porque el
backend solo recoge de Drive los slots que el expediente declara — la foto quedaría invisible.
El alcance de foto de un RES080 sale de la pestaña **Envolvente** del expediente
(`documentacion.envolvente`: `sustituye_ventanas`, `aislamiento_cubierta`, `aislamiento_muros`), que
activa los apartados automáticamente en cuanto se guarda. Así que:
- Comprueba `documentacion.envolvente` en la query del paso 0.1. Si declara la actuación y aun así no
  sale el slot, vuelve a llamar a `estado_anexo_fotografico` (la propia consulta reconcilia el alcance).
- Si la Envolvente **no** la declara, registra una incidencia LEVE indicando la acción concreta:
  *"La factura incluye <actuación> pero el expediente no tiene ese apartado de foto. Marcarlo en la
  pestaña Envolvente (o con «Añadir apartado de obra» en el gestor de documentación) y regenerar."*
  No hay herramienta para activarlo desde aquí; es una acción del equipo en la app.
...
- `FOTO_CUBIERTA_ANTES/DESPUES`, `FOTO_FACHADA_ANTES/DESPUES`, `FOTO_SUELO_ANTES/DESPUES`,
  `FOTO_PLACAS_SOLARES` según lo que devuelva `slots[]` — cada uno solo si su actuación está justificada.

En cualquier caso, **la lista buena es la de `slots[]`**, no esta: aquí solo están los habituales.
...
- Slot con varias (`multiple=true`): `<SLOT>_<N>.<ext>`, empezando por el **`siguiente_indice`** que te
  dio `presentes[]` para ese slot (o por `1` si el slot está en `faltan`), e incrementando por cada foto
  que copies.
  ⚠️ **NUNCA calcules el índice a partir de `count`.** Los índices en Drive pueden tener huecos
  (`_1`, `_2`, `_5` → `count` = 3, pero el siguiente libre es el 6) y **Google Drive admite dos ficheros
  con el mismo nombre en la misma carpeta**: no da error, se queda con los dos y la foto saldría
  **duplicada** en el anexo.
  Si `estado_anexo_fotografico` no te devuelve `siguiente_indice` ni `nombres[]` (backend aún sin
  actualizar), sácalo del listado de `12. DOCUMENTOS PARA CEE` que ya hiciste en el paso 2: coge los
  ficheros que empiezan por `<SLOT>_`, quédate con el mayor número y súmale 1.
...
## Checkpoint de continuación (SIEMPRE al terminar — optimización de tokens)
Para no re-derivar el estado en la próxima sesión (ahorra tokens y tiempo), deja un resumen corto de **por dónde nos hemos quedado, qué falta y quién lo desbloquea**, en la BD y como ÚLTIMO mensaje del chat.
1. **Al EMPEZAR**, lee primero `expedientes.seguimiento->'checkpoint'` (vía `execute_sql`). Si existe y sigue vigente, retoma desde ahí.
2. **Al TERMINAR**, escríbelo (merge; NO toca el resto de `seguimiento`):
```sql
update expedientes set seguimiento = coalesce(seguimiento,'{}') || jsonb_build_object(
  'checkpoint', jsonb_build_object(
    'at','<AAAA-MM-DD>', 'por','generar-anexo-fotografico',
    'estado','<1 línea: en qué punto está>',
    'siguiente_paso','<1 línea accionable para continuar>',
    'responsable','<BROKERGY|INSTALADOR|CLIENTE|CERTIFICADOR>',
    'falta', jsonb_build_array('<ítem>','<ítem>')))
where id=:exp_id;
```
3. Cierra el chat con ese mismo resumen en 1-2 líneas. Ej.: *"Anexo Fotográfico generado y enlazado; falta la foto del hueco de la caldera. Continuar: pedirla al INSTALADOR y enviar a firmar."*

## Ajustes manuales del anexo — RESPÉTALOS

El equipo puede haber afinado el anexo a mano desde el gestor de fotos de la app. Esas decisiones viven
en `expedientes.documentacion` y **el backend las aplica al generar**, así que tú no tienes que hacer
nada… salvo no contradecirlas:

- **`anexo_excluidas`** `[nombres de fichero]` — fotos que alguien decidió **dejar fuera del anexo**. El
  fichero SIGUE en Drive, así que te aparecerá en `presentes[]` y en el listado de la carpeta: **no la
  vuelvas a copiar, no la cuentes como incidencia y no la "recuperes"**. Es una decisión deliberada.
- **`anexo_orden`** `{SLOT: [nombres…]}` — orden manual de las fotos dentro de un concepto. Si copias
  fotos nuevas a ese slot se colocarán al final, que es lo correcto: no toques este campo.
- **`anexo_comentarios`** `{SLOT: texto}` — textos explicativos que se imprimen en el documento. No los
  escribas ni los modifiques desde aquí; los redacta el equipo.

**Consecuencia práctica para el informe final:** `numPhotos` que devuelve `generar_anexo_fotografico`
puede ser **menor** que el número de fotos que ves en Drive, porque las excluidas no entran. No es un
error ni una foto perdida — si hay diferencia, menciónala como "N fotos excluidas del anexo a mano".

Consúltalos en la query del paso 0.1 añadiendo
`e.documentacion->'anexo_excluidas'`, `e.documentacion->'anexo_orden'` y `e.documentacion->'anexo_comentarios'`.
